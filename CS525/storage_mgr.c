...
RC readBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage)
{
...

    {
           ...
           readResult= fread(memPage, sizeof(char),PAGE_SIZE,fHandle->mgmtInfo);
            ...
            {
                printf("error in fread,size is lesser than page size\n");
    ...
    {   
        return RC_FILE_NOT_FOUND;
        printf("mgminfo is null,check the readblock\n");
    }
...
RC appendEmptyBlock (SM_FileHandle *fHandle){
    ...
    if(result!=0)
        ...
        free(buff);
        return RC_OK;                                       //if could not write into the file return WRITE_FAILED ERROR.
        }
    else
    {
        return RC_WRITE_FAILED;
